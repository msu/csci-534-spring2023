...
Let $P$ be a polygon in $\R^2$.
How many vertices and edges does the dual graph of a frugal triangulation of $P$
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\collab{\todo{}}
\nextprob{Randomized Convex Hull}

\begin{enumerate}

    \item Describe and analyze an algorithm that computes the
        convex hull of a set of $n$ points in the plane using randomized
        incremental construction in expected $\Theta(n \log n)$ time. For this
    ...
    \item Give an example of a set of points in the plane, and a
        particular input order, that causes the convex hull algorithm to run in
        $\Theta(n^2)$ when the points are added in this particular order. Make sure it
